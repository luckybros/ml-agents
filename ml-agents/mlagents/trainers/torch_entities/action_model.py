...
class ActionModel(nn.Module):
    ...
    def evaluate(
        ...
        #.info(f"conditional_sigma: {self.conditional_sigma}")
    ...
    def forward(
        ...
        """
        #logger.debug("--- ACTION MODEL DEBUG ---")
        dists = self._get_dists(inputs, masks)
        actions = self._sample_action(dists)
        log_probs, entropies = self._get_probs_and_entropy(actions, dists)
        #logger.debug(f"dists in ActionModel: {dists}")
        #logger.debug(f"actions in ActionModel: {actions}")
        #logger.debug(f"log_probs in ActionModel: {log_probs}")
        # Use the sum of entropy across actions, not the mean
        entropy_sum = torch.sum(entropies, dim=1)
